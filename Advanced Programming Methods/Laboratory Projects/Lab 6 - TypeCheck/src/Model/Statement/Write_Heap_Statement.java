...
public class Write_Heap_Statement implements IStatement{
    ...
    public My_I_Dict<String, IType> typeCheck(My_I_Dict<String, IType> typeEnv) throws Statement_Execution_Exception, Expression_Evaluation_Exception, ADT_Exception {
        if (typeEnv.lookUp(variableName).equals(new Ref_Type(expression.typeCheck(typeEnv))))
            return typeEnv;
        else
            throw new Statement_Execution_Exception("ERROR: WriteHeap: right hand side and left hand side have different types.");
    }
