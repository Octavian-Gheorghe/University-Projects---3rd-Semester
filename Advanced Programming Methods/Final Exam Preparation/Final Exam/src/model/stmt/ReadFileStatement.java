...
public class ReadFileStatement implements IStmt{
    ...
    public IStmt deepcopy() {
        return new ReadFileStatement(exppression, variable_name);
    }

    @Override
    public MyIDictionary<String, Type> typecheck(MyIDictionary<String, Type> typeEnv) throws MyException {
        Type typeExp = exppression.typecheck(typeEnv);
        Type typeVar = typeEnv.lookup(variable_name);
        if(! (typeExp.equals(new StringType())))
            throw new MyException("The type of the expression must be string!");
        if ( ! (typeVar.equals(new IntType())))
            throw new MyException("The type of the variable should be int!");
        return typeEnv;
    }
