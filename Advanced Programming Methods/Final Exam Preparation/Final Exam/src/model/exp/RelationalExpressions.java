...
public class RelationalExpressions implements Exp{
    ...
    public Type typecheck(MyIDictionary<String, Type> typeEnv) throws MyException {
        Type typeExp1 = firstExpression.typecheck(typeEnv);
        Type typeExp2 = secondExpression.typecheck(typeEnv);
        if (typeExp1.equals(new IntType())) {
            if (typeExp2.equals(new IntType()))
                return new BoolType();
            else
                throw new MyException("Second operand is not Int type!");
        } else
            throw new MyException("First operand is not Int type!");
    }
}
